...
Stage 0 repaired legacy CSV curve handling. `pnl_json` survives legacy CSV pre-filtering solely to be compressed into the local correlation-curve cache, while the primary Factor Center record retains only the cache reference and boolean state. The local smoke fixture now stores its cache in a temporary directory; no runtime cache is retained in the source worktree.
