...
2026-07-18 架构审计确认：当前没有可复用的 DB Provider Profile、Secret Vault 或无人值守 Provider Preflight 生命周期。后续应新增 ProviderProfile/Secret 机制，前端只读取 configured/masked/status/last_validated_at；API Key、Base URL、Cookie、Token 不得进入 Git、日志、Context Snapshot、Research Package 或 wq-review。
...
V1 provides versioned `ResearchSkillSpec` records, auditable model invocations, Provider Profile preflight, and an encrypted Provider Secret database implementation. The five required core-table conclusion still treats Provider Secret as a database-table or external-store choice; this delivery selected the database-table option. API views return only masked/configuration status fields.

Two deterministic fixture Skills, `fixture.round_rank@1` and `fixture.round_mean@1`, exist solely for the local Stage 7 acceptance campaign. They emit bounded CandidateProposal data and have no provider, Gate, scheduler, database-administration, or direct execution capability. Real provider connectivity is not validated by the fixture path.
