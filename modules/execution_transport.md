...
V1 does not add a transport. `ResearchOrchestrator` calls only the existing shared mock dispatcher with `agent_only=True`, then waits for existing execution requests and Result Ingestion facts. Gate closure blocks dispatch while preserving already materialized candidates for later recovery. The Stage 7 campaign verified restart/recovery and a duplicate result callback without creating a second request or result ledger. No real adapter, Prod Corr path, or final-submit path was invoked.
