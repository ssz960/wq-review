...
Before V1 materialization, an unbound CandidatePlan is bound once to the active Registry snapshot and Simulation Profile; hard admission then validates the concrete fields, operator, and settings against that immutable scope. The local Stage 7 campaign imported and activated a fixture snapshot, admitted twelve candidates, and rejected one unavailable-field proposal. This is local fixture evidence only and does not prove server Registry freshness.
