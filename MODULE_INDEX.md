...
| allocation_decision | [allocation_decision_matrix](modules/allocation_decision_matrix.md) | `backend/app/mining/single_multi_allocation.py` | Historical decision matrix; current authority remains the deterministic allocator. |
| capacity_policy | [capacity_policy_and_slot_model](modules/capacity_policy_and_slot_model.md) | `backend/app/models.py` | Shared/separate/unknown capacity policy and reservation ledger. |
| scheduler_recovery | [scheduler_recovery_state_machine](modules/scheduler_recovery_state_machine.md) | `backend/app/mining/single_multi_allocation.py` | Lease recovery and idempotent resume contract. |
...
| ai_mode | [ai_mode](modules/ai_mode.md) | `backend/app/mining/agent_runtime_v1.py`、`provider_preflight.py` | Agent 与 supervisor 两套语义并存；预检使用独立单入口。 |
...
| provider_and_skills | [provider_and_skills](modules/provider_and_skills.md) | `backend/app/config.py`、`backend/app/mining/*provider*.py` | 独立预检链已实现；服务器真实调用仍被 SSH 阻断。 |
