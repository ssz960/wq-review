...
| `llm_provider_secrets` | NEW or external Secret Store | No encrypted DB secret/ref model exists. Env-only is not enough for unattended profiles. | Encrypted secret material or secret reference metadata. | `secret_ref`. | active, rotated, revoked, invalid. | Rotate/revoke; hard deletion only after no profile references. |

Candidate new table count after reuse analysis: 6 (`research_rounds`, `research_hypotheses`, `memory_proposals`, `research_assistance_requests`, `llm_provider_profiles`, `llm_provider_secrets`). `research_campaign_events` should start as `MiningEvent` extension/projection, not a new table.
